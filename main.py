...
# -----------------------------------------------------------
# --- نظام البروكسي لتخطي حظر منافذ SMTP في الاستضافات ---
# إذا استمر خطأ timeout على Railway، يمكنك تفعيل البروكسي
# وتوفير عنوان SOCKS5 شغال لتجاوز حظر المنافذ:
# import socks
# PROXY_IP = "ضع_اي_بي_البروكسي"
# PROXY_PORT = 1080
...
def connect_gmail_smtp(email, password, retries=2):
    ...
    for attempt in range(retries):
        # المحاولة 1: عبر المنفذ 587 (TLS - الأكثر توافقاً)
        try:
            ...
            last_exception = f"Port 587: {e}"

        # المحاولة 2: عبر المنفذ 465 (SSL)
        try:
            server = smtplib.SMTP_SSL('smtp.gmail.com', 465, context=context, timeout=timeout_seconds)
            server.login(email, password)
            return server
        except Exception as e:
            last_exception = f"{last_exception} | Port 465: {e}"

        time.sleep(1)

    raise Exception(f"تعذر الاتصال بـ Gmail (تحقق من حظر المنافذ في الاستضافة): {last_exception}")
...
def verify_senders_process(message, data):
    ...
    for sender in senders:
        try:
            ...
            invalid_accounts.append(f"{sender['email']} (الخطأ: {str(e)[:40]})")
