...
TOKEN = '8951275205:AAGTA5sjicnNCSf2WtyXu57IS-w51YxhTsc'
...
# --- محرك الاتصال بـ Gmail (محدث: تجاوز شهادات الأمان لحل مشكلة الـ Timeout على الهاتف) ---
...
def connect_gmail_smtp(email, password, retries=3):
    ...
    for attempt in range(retries):
        # المحاولة 1: الاتصال عبر Port 587 (TLS)
        try:
            ...
            last_exception = f"Port 587 error: {e}"

        # المحاولة 2: الاتصال عبر Port 465 (SSL)
        try:
            server = smtplib.SMTP_SSL('smtp.gmail.com', 465, context=context, timeout=timeout_seconds)
            server.login(email, password)
            return server
        except Exception as e:
            last_exception = f"{last_exception} | Port 465 error: {e}"
...
def update_progress_msg(chat_id, message_id, results_dict, total_sends, mode_text):
    last_success = -1
    last_failed = -1
    while not results_dict.get('is_done', False):
        if results_dict['success'] != last_success or results_dict['failed'] != last_failed:
            ...
            text = (
                ...
                f"⏳ يرجى الانتظار، سيتم التحديث تلقائياً..."
            )
            try:
...
def single_sender_task(message, sender, recipients, start_recipient_idx, start_delay_idx, subject, body_text, count, smart_delays, fixed_delay, image_path, results_dict, counter_lock):
    ...
    for i in range(count):
        ...
        if current_delay > 0 and not results_dict.get('is_done', False):
            time.sleep(current_delay)
    
    try: server.quit()
    except: pass

def sending_thread_individual(message, senders, recipients, subject, body_text, count, smart_delays, fixed_delay, image_path, status_msg, total_sends, mode_text):
    results = {'success': 0, 'failed': 0, 'is_done': False}
    counter_lock = threading.Lock()
    
    threading.Thread(target=update_progress_msg, args=(message.chat.id, status_msg.message_id, results, total_sends, mode_text)).start()
...
def sending_thread_all(message, senders, recipients, subject, body_text, count, smart_delays, fixed_delay, image_path, status_msg, total_sends, mode_text):
    results = {'success': 0, 'failed': 0, 'is_done': False}
    counter_lock = threading.Lock()
    ...
    for sender in senders:
        t = threading.Thread(target=single_sender_task, args=(message, sender, recipients, recipient_idx, delay_counter, subject, body_text, count, smart_delays, fixed_delay, image_path, results, counter_lock))
        threads.append(t)
        t.start()
        recipient_idx += count
        delay_counter += count
...
def start_sending_process(message, user_id, mode):
    ...
    thread = threading.Thread(
        target=target_func, 
        args=(message, list(data['senders']), list(data['recipients']), data['subject'], data['message'], count, list(data['smart_delays']), data['delay'], data['image_path'], status_msg, total_sends, mode_text)
    )
    thread.start()
