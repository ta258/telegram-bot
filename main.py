...
# --- إجبار بايثون على استخدام IPv4 فقط لحل مشكلة Errno 101 ---
...
# --- محرك الاتصال بـ Gmail (مُعدّل لمنع الفشل) ---

def connect_gmail_smtp(email, password, retries=2):
    """دالة تتصل بسيرفر Gmail عبر SSL أو TLS وتتعامل مع أخطاء الشهادات والشبكة"""
    ...
    for attempt in range(retries):
        ...
        # المحاولة 2: الاتصال عبر Port 465 (SSL) في حال كان 587 محجوباً
        try:
...
def callback_handler(call):
    ...
    if action == "add_sender":
        ...
        msg_text = "اضف ايميلات الارسال بالصيغة التالية:\n\nالايميل:الباسورد\n\nمثال:\nGis@gmail.com:password123"
...
def step_get_senders_list(message):
    ...
    if not matches and text:
        ignored = len(text.split("\n"))

    save_user_data(user_id, data) 
    response_msg = f"✅ تم حفظ {added} حساب مرسل بنجاح."
    if ignored > 0: response_msg += f"\n⚠️ تم تجاهل {ignored} بسبب صيغة غير صحيحة أو لتكرارها."
...
def single_sender_task(message, sender, recipients, start_recipient_idx, start_delay_idx, subject, body_text, count, smart_delays, fixed_delay, image_path, results_dict, counter_lock):
    ...
    try:
        # استخدام دالة الاتصال المضمونة
        server = connect_gmail_smtp(sender_email, sender_password)
    except Exception as e:
        err_msg = str(e)
        if "Authentication failed" in err_msg or "5.7.8" in err_msg or "Invalid Second Factor" in err_msg:
            reason = "كلمة مرور التطبيقات (App Password) غير صحيحة"
        else:
            reason = f"مشكلة اتصالات: {err_msg[:30]}"
